...
const int encoder_FL = 3; // forward left interrupt
const int encoder_FR = 2; // forward right interrupt
const int encoder_BL = 18; // backward left interrupt
const int encoder_BR = 19; // backward right interrupt

long long int count_FL = 0;
long long int count_FR = 0;
long long int count_BL = 0;
long long int count_BR = 0;

const int LF_PWM = 7; //blue color wire
const int RF_PWM = 6; // green color wire
const int LB_PWM = 5;
const int RB_PWM = 4;

const int LF_BACK = 30; //purple color wire
const int LF_FORW = 31;

const int RF_BACK = 32;
const int RF_FORW = 33; // black color wire


const int LB_BACK = 34;
const int LB_FORW = 35;

const int RB_BACK = 36;
const int RB_FORW = 37;
bool wtf;

// encoder code for the interrupt
void counter_FL(){
...
void setup() {
  ...
  setpins();
  attachInterrupt(digitalPinToInterrupt (encoder_FL), counter_FL, RISING);
  attachInterrupt(digitalPinToInterrupt (encoder_FR), counter_FR, RISING);
  attachInterrupt(digitalPinToInterrupt (encoder_BL), counter_BL, RISING);
  attachInterrupt(digitalPinToInterrupt (encoder_BR), counter_BR, RISING);
...
float ticks_per_meter = 106.103;

void loop() {
  
  // count encoder ticks
  long long ct1 = count_FL;
  long long ct2 = count_FR;
  long long ct3 = count_BL;
  long long ct4 = count_BR;
